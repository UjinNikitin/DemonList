...
public:
	...
	void Add(int a)
	{
		...
		if (a == 2) //В КОНЕЦ списка
		{
			if (Head) //Если список не пуст
			{
				...
				Tail->next = new Chain;//Там де был указатель на NULL теперь будет указатель на какой-то элем.
	...
	void StreamInput()
	{
		...
		//Костыль для улаления пустых элем. в начале и в конце списка (появляются если стрим инпуте ввести сначала пробел а потом слово
	...
	while (work)
	{
		...
		switch (c)
		{
			case '1':
			{
				...
				cout <<"Ваш елемент находится между элементами, которые находятся в "<< element->prev <<" и "<<element->next<<" ячейках памяти"<<endl;
				_getch();
				break;
			}
			case '2':
			...
			case '3':
			{
				...
				while (!(cin >> a) || a<1 || a>2)				//Весь етот вайл - простой cin,
				{												//но модифицырованый для работы в switch и других методах класса
					cin.clear();								//ибо простой cin тупо не работает, долго пояснять почему
					while (cin.get() != '\n');					//ну и тут есть перехват, но это уже второстепенно
					cout << "Некорректный ввод! Повторите!\n";	//
				}												//
